...
Note C: it is incorrect that there was no shared email for the sktime council at the time of minuting, this may be seen as implied by MR's answer. However, it existed, and the email was `sktime.toolbox@gmail.com`. MR is imprecise here, what is correct: the email did exist, but MR had joined the council recently and did not have *access* to the email yet (and there were simultaneous internal discussions on possibly moving to another email on the `sktime` webdomain).
